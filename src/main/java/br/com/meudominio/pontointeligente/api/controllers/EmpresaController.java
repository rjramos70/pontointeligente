...
import br.com.meudominio.pontointeligente.api.dtos.EmpresaDto;
...
public class EmpresaController {
	...
	public ResponseEntity<Response<EmpresaDto>> buscarPorCnpj(@PathVariable("cnpj") String cnpj){
		...
		if (!empresa.isPresent()) {
			...
			return ResponseEntity.badRequest().body(response);		// retorna um status 400 (badRequest)
		}
		
		response.setData(this.converteEmpresaDto(empresa.get()));
		return ResponseEntity.ok(response);		// retorna um status 200
